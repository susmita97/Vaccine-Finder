...
if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true){
  header("location: homepagepatients.php");
  exit;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($sub_email_err) && empty($sub_password_err)){
        ...
        $sql = "select patient_id,email,password from patients where email = ? and password = ?";
        ...
        if($stmt = mysqli_prepare($conn, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                mysqli_stmt_store_result($stmt);
                    ...
                    mysqli_stmt_bind_result($stmt, $patientid, $email, $hashed_password);
                    if(mysqli_stmt_fetch($stmt)){
                            ...
                            $sql2="insert into sessions_patients(p_id,logged_in_at) values('$patientid',CURRENT_TIMESTAMP)";
							if(mysqli_query($conn, $sql2)){
								echo "<script>alert('please fill all the fields $patientid')</script>";
							} 
							else{
...
<div class="container">
    <div class="row">
		    ...
		    <h4 class="left-align teal-text"><br>Don't have an account? Create One.</h4><br>
